  ...
  async run (message, args, { t }) {
    guild.findOne({ _id: message.guild.id }, async (err, database) => {
      if (err) console.log(err)
        ...
        const collector = msg.createReactionCollector((r, u) => (r.emoji.name === '🇧:regional_indicator_r:', '🇺:regional_indicator_s:') && (u.id !== client.user.id && u.id === message.author.id))

        collector.on('collect', async r => {
          switch (r.emoji.name) {
            case '🇧:regional_indicator_r:':
              await msg.edit(new Discord.RichEmbed().setDescription('Agora vou falar em português!').setColor('#42f445'))
              database.language = 'pt-BR'
              database.save()
              break

            case '🇺:regional_indicator_s:':
                await msg.edit(new Discord.RichEmbed().setDescription('Now I\'ll speak english!').setColor('#db3939'))
