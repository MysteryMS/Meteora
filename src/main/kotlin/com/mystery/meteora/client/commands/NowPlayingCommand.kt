...
class NowPlayingCommand(ctx: MessageReceivedEvent, args: String, prefix: String, config: Config) :
    ...
    when {
      ...
      guildPlayer.player.playingTrack == null -> context.channel.sendMessage(
        ...
        val embed = EmbedBuilder()
          ...
          .addField(
            "nowplaying.length".translate(config, context.guild.id),
            "${Parser().parse(track.position)}/${Parser().parse(track.duration)}",
            false
          )
          ...
          .addField(
            "nowplaying.requestedBy".translate(config, context.guild.id),
            "<@${guildPlayer.trackScheduler.requestedByAuthorId}>",
            false
          )
