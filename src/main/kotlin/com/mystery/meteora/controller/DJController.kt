...
class DJController() {
  fun hasDjRole(context: MessageReceivedEvent): Boolean {
    ...
    client.close()
    return context.member?.roles?.contains(role)!! || context.member!!.user.idLong == PlayerController(context).manager.trackScheduler.requestedByAuthorId || context.member!!.hasPermission(Permission.ADMINISTRATOR)
  }
}